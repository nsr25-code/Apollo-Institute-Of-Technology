...
const BranchSelector: React.FC<BranchSelectorProps> = ({ dept, onSelect, onApply, onClose }) => {
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 md:p-8">
      ...
      <div className="relative w-full max-w-6xl bg-slate-900 border border-slate-800 rounded-[3rem] overflow-hidden shadow-2xl animate-in zoom-in duration-500 flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="p-8 md:p-12 border-b border-slate-800 flex justify-between items-center bg-slate-900/80 backdrop-blur-md sticky top-0 z-20">
          <div className="flex items-center gap-6">
             <div className="p-4 rounded-2xl bg-blue-600/10 border border-blue-500/20 text-blue-500">
             ...
             <div>
               ...
               <p className="text-slate-400 font-space uppercase text-xs tracking-[0.3em]">Select your engineering vertical</p>
             </div>
          </div>
          <button 
            onClick={onClose}
            className="p-4 bg-slate-800 hover:bg-red-500/20 hover:text-red-400 rounded-full text-slate-400 transition-all border border-slate-700 hover:border-red-500/50 group"
          >
        ...
        {/* Branch Grid */}
        <div className="p-8 md:p-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 overflow-y-auto blueprint-grid flex-1 scrollbar-custom">
              ...
              <div className="p-8 flex-grow flex flex-col justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-[10px] font-space font-bold text-slate-500 uppercase tracking-[0.2em]">Department Course</span>
                  </div>
                  <h3 className="text-2xl font-bold font-space text-white mb-4 group-hover:text-blue-400 transition-colors">
                    {branch.name}
                  </h3>
                  <p className="text-sm text-slate-400 font-space leading-relaxed mb-8">
                    {branch.description}
                  </p>
                </div>
        ...
        {/* Footer */}
        <div className="p-8 bg-slate-950/80 text-center text-slate-500 text-[10px] font-space uppercase tracking-[0.4em] border-t border-slate-800 backdrop-blur-md">
